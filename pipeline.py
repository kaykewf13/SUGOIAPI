...
- Sintaxe Python válida (aspas, __file__, __name__, indentação)
- Sem blocos ``` dentro do código
- Classificação por URL/padrões e filtros PT/BR mantidos
"""
...
SOURCE_CANAIS_BR = "https://raw.githubusercontent.com/Free-TV/IPTV/master/playlist.m3u8"
...
def tem_portugues(nome: str, url: str, extinf: str) -> bool:
    ...
    if any(t in e for t in [
        ...
        'tvg-language="pt"', 'tvg-country="br"', 'tvg-country="pt"',
    ]):
        return True
    ...
    if any(d in u for d in [
        ...
        "logicahost.com.br", "camara.gov", "senado.leg",
    ]):
        return True

    # Domínio português
    if any(d in u for d in ["rtp.pt", "tvi.pt", "sic.pt", "cmtv.pt", "record.pt"]):
        return True
...
def is_canal_brasileiro(nome: str, url: str, extinf: str) -> bool:
    ...
    if any(d in u for d in [
        ".com.br", ".gov.br", "logicahost.com.br",
...
SERIES_KEYWORDS = [
    "SERIE", "TEMPORADA", "EPISODIO", "EP.", " S0", " S1", " S2",
    "SEASON", "EPISODE", "OVA",
]
...
CATEGORIAS_FILMES = {
    ...
    "Adulto": ["HENTAI", "ADULT", "EROTIC", "XXX", "PORN", "AV", "UNCENSORED"],
    "Geral": [],
}


def detectar_categoria(nome_upper: str, mapa: dict[str, list[str]]) -> str:
...
def classificar_item(nome: str, url: str) -> dict[str, str]:
    n = (nome or "").upper()
    u = (url or "").lower()

    # Canal: por keyword no nome OU por padrão de URL ao vivo
    if any(k in n for k in LIVE_KEYWORDS) or any(p in u for p in LIVE_URL_PATTERNS):
...
def parse_serie(nome: str) -> dict[str, str]:
    nome = nome or ""
...
def listar_arquivos_repo() -> list[str]:
    url = (
...
def extrair_links(raw_url: str, filtro_br: bool = False, filtro_pt: bool = False) -> list[dict]:
    try:
        ...
        for i, line in enumerate(lines):
            ...
            last_seg = (url.split("/")[-1] if url else "").lower()
            if re.match(r"^[\w\-]+\.m3u8?$", nome_lower) or re.match(r"^[\w\-]+\.m3u8?$", last_seg):
                continue

            # Descarta nomes muito curtos
            if len(nome.strip()) < 3:
                continue
...
def validar_em_paralelo(acervo: list[dict], workers: int = 40) -> list[dict]:
    validos: list[dict] = []
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futuros = {ex.submit(link_esta_vivo, item["URL"]): item for item in acervo}
...
def gerar_m3u(validos: list[dict]) -> Path:
    # Deduplica por URL
    vistos: set[str] = set()
    unicos: list[dict] = []
    for item in validos:
    ...
    # Classifica
    for item in unicos:
        item.update(classificar_item(item.get("Nome", ""), item.get("URL", "")))

    # Separa grupos
    canais = sorted(
        [i for i in unicos if i.get("grupo") == "Canais"],
        key=lambda x: (x.get("Nome", "") or "").upper(),
    )
    filmes = sorted(
        [i for i in unicos if i.get("grupo") == "Filmes"],
        key=lambda x: (x.get("categoria", "Geral"), (x.get("Nome", "") or "").upper()),
    )

    series = [i for i in unicos if i.get("grupo") == "Series"]
    for s in series:
        s.update(parse_serie(s.get("Nome", "")))
    series.sort(key=lambda x: (
        x.get("categoria", "Geral"),
        (x.get("titulo", "") or "").upper(),
        x.get("temporada", "Temporada 01"),
        x.get("episodio", "Episodio 001"),
    ))
    ...
    with open(m3u_path, "w", encoding="utf-8") as f:
        ...
        for item in canais:
            nome = re.sub(r"[^\w\s\-]", "", item.get("Nome", "")).strip() or "Canal"
        ...
        cat_atual = None
        for item in series:
            if item.get("categoria") != cat_atual:
                cat_atual = item.get("categoria", "Geral")
                f.write(f"\n## ── {cat_atual} ──\n\n")

            titulo = item.get("titulo", item.get("Nome", ""))
            temporada = item.get("temporada", "Temporada 01")
            episodio = item.get("episodio", "Episodio 001")
            tvg_name = f"{titulo} | {temporada} | {episodio}"

            f.write(
                f'#EXTINF:-1 tvg-name="{tvg_name}" tvg-type="series" '
                f'group-title="Series | {cat_atual} | {titulo} | {temporada}", {episodio}\n'
            )
            f.write(f'{item.get("URL","")}\n\n')
        ...
        for item in filmes:
            ...
            nome = re.sub(r"[^\w\s\-]", "", item.get("Nome", "")).strip() or "Filme"
            f.write(
